...
17. Spring Boot와 함께 하는 Spring AOP배우기

    1. AOP용어 훑어보기
   	1. 컴파일 단계
	       Advice: 실행할 코드를 의미 - 로깅, 인증하기
	       Pointcut: 인터셉트하려는 메소드 호출을 의미
	       Aspect: Advice와 Pointcut의 조합
	       Weaver: AOP를 구현한 프레임워크를 의미
     	2. 런타임 단계
               Join Point: 포인트컷 조건이 참이면 어드바이스가 실행된다. - 어드바이스 실행 인스턴스를 가리킴
    2. AOP 어노테이션
       	@Before 메소드 호출전에 실행 - 메소드가 호출되기전 로그를 출력, 또는 인증이 필요할때 유용하다
        @After 메소드가 실행되고 난 뒤 수행할 작업을 지정한다 메소드의 결과가 성공이든 실패든 익셉션을 던지든 상관없이 실행
        @AfterReturning 메소드가 성공적으로 실행됐다면 하려는 작업을 명시 익셉션 발생시 수행X
        @AfterThrowing 메소드가 익셉션을 던졌을 때 뭔가를 해야할 때 씀 메소드가 값을 성공적으로 반환시 수행X
        @Around 메소드 실행 전과 후에 뭔가를 한다. 
